...
\end{align}
Let us prove the opposite inequality. We divide the proof in two steps: First, we treat the case when $H(x,\cdot)$ is strongly radially increasing. Then, we will deal with the general case.\\

\noindent
\textit{Step 1:} $H(x,\cdot)$ strongly radially increasing on $\R^\alpha$.\\ 
We start by setting $$M:=\esssup_U H(\cdot, \mathrm D ^{[k]}u).$$
