...
The second assumption on $H$ concerns the uniformity w.r.t. $x$ of its modulus continuity: since $H(x,\cdot)$ is continuous in $\R^\alpha$, it is uniformly continuous on every ball $B_R(0)\subset\R^\alpha$. Thus, we require that the modulus of continuity on $B_R(0)$ is independent of $x$. More precisely, we assume
\begin{equation}\label{uniformity}
\begin{array}{l}
\forall R>0 \,\,\exists\, \omega_R\in C(\mathbb{R}^+)\ \text{non-decreasing, with } \omega_R(0)=0: \\
\multicolumn{1}{c}{|H(x,X)-H(x,X')|\leq\omega_R(|X-X'|),\qquad \text{a.e. } x\in\Omega,\quad\forall X\in B_R(0)}.
\end{array}
\end{equation}
...
\end{align}
Let us prove the opposite inequality. To this purpose, set $$M:=\esssup_U H(\cdot, \mathrm D ^{[k]}u).$$
If $M=0$, there is nothing to prove, so we can suppose that $M>0$.
...
$$
Notice that $u_\lambda\in W^{k,\infty}{(U,\R^N)}$. Moreover, $u_\lambda=u$ on $U^{\eps/2}\cap U$, so that $u_\lambda\in u+W^{k,\infty}_c(U,\R^N)$.\\
