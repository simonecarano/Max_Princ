...
$$
Under very mild assumptions on the supremand $\rom{H}$, we prove that for an absolute minimiser the essential supremum in \eqref{E infty} is ``attained at the boundary of $U$". In order to give a precise meaning to this expression, we refer to the supremum on $\partial U$ of the \textit{essential limsup} of $\rom{H}(\cdot, \mathrm D ^{[k]}u)$, which is defined as
...
Now we proceed to specify the assumptions on the function $\rom{H}$. By denoting $X=(X_0,X_1,\ldots,X_k)\in R^N\times \R^{Nn}\times\ldots\R^{Nn^k}=\R^\alpha$, we assume that $\rom{H}:\Om\times\R^\alpha\to\R$ is Carathéodory and radially non-decreasing w.r.t. $X$, i.e. for almost every $x\in\Om$ and every $X\in\R^\alpha$ the function
$$
t\mapsto \rom{H}(x,tX) \mbox{ is non-decreasing in }\R^+.
$$
Notice that radial monotonicity implies that, for almost every $x\in\Om$, $\rom{H}(x,\cdot)$ has a global minimum at $X=0$. Without loss of generality we can assume that $\rom{H}(x,0)=0$ a.e. $x\in\Om$, so that $\rom{H}$ takes values in $\R^+$. In this case, $E_\infty(u,U)=\|\rom{H}(\cdot, \mathrm D^{[k]}u)\|_{L^\infty(U)}$ for every $U\subseteq\Om$ open.

We point out that any level convex function with global minimum at the origin is necessarily radially non-decreasing, so that our result applies to the class of level convex functions, which is a common assumption for $\rom{H}$ in $L^\infty$ variational problems (see e.g. \cite{BJW}).
...
The second assumption on $\rom{H}$ concerns the uniformity w.r.t. $x$ of its modulus continuity: since $\rom{H}(x,\cdot)$ is continuous in $\R^\alpha$, it is uniformly continuous on every closed ball $\overline{\mathbb B}_R(0)\subset\R^\alpha$. Thus, we require that the modulus of continuity on $\overline{\mathbb B}_R(0)$ is independent of $x$. More precisely, we assume
...
Let $\Om\subset\R^n$ be a bounded open set and let $\rom{H}:\Om\times\R^\alpha\to\R^+$ be a Carathéodory function satisfying \eqref{uniformity}, with $\rom{H}(x,0)=0$ and $\rom{H}(x, \cdot)$ radially non-decreasing for a.e. $x\in\Om$. Assume that $u\in W^{k,\infty}(\Om,\R^N)$ is an absolute minimiser for the functional $E_\infty$, defined in \eqref{E infty}. Then $u$ satisfies the maximum principle for $E_\infty$ in $\Om$, namely
\begin{align}\label{MP}
\esssup_U \rom{H}(\cdot, \mathrm D ^{[k]}u)=\sup_{\partial U}\rom{H}(\cdot,\mathrm D ^{[k]}u)^*\qquad\forall U\subseteq\Om \mbox{ open}.
\end{align}
...
Let $\Om\subset\R^n$ be a bounded open set and let $\rom{H}:\Om\times\R^\alpha\to\R^+$ be a Carathéodory function satisfying \eqref{uniformity}, with $\rom{H}(x,0)=0$ and $\rom{H}(x, \cdot)$ radially non-decreasing for a.e. $x\in\Om$. Consider the functional $E_\infty$, defined in \eqref{E infty}.
...
\end{align}
Let us prove the opposite inequality. We divide the proof in two steps: First, we treat the case when $\rom{H}(x,\cdot)$ is strongly radially increasing, uniformly in $x$. Then, we will deal with the general case.\\

\noindent
\textit{Step 1:} $\rom{H}(x,\cdot)$ strongly radially increasing on $\R^\alpha$, uniformly w.r.t $x\in \Omega$.\\ 
...
$$
\lim_{\rho\to0}\esssup_{U^\rho\cap U}\rom{H}(\cdot, \mathrm D ^{[k]}u)+2\delta\leq M.
$$
In particular, there exists $\eps>0$ such that
\begin{align}\label{from contr}
\esssup_{U^{\eps}\cap U}\rom{H}(\cdot, \mathrm D ^{[k]}u)+\delta\leq M.
\end{align}
...
\begin{align*}
\esssup_{U^\eps\cap U}\rom{H}(\cdot, \mathrm D^{[k]}u_\lambda)&\leq\esssup_{U^\eps\cap U}\rom{H}(\cdot, \lambda\mathrm D^{[k]}u)+ \omega\left((1-\lambda)C_2\right)\\
...
\esssup_{U^\eps\cap U}\rom{H}(\cdot, \mathrm D^{[k]}u_\lambda)\leq M-\frac{\delta}{2}<M.
\end{align}
On the other hand, let us prove that
\begin{align}\label{second key}
\esssup_{U\setminus U^\eps}\rom{H}(\cdot, \mathrm D^{[k]}u_\lambda)<M.
\end{align}
To this purpose, notice that, since $\varphi_\eps=0$ in $U\setminus U^\eps$, we have $u_\lambda=\lambda u$ in $U\setminus U^\eps$. Thus, set $$\overline M:=\esssup_U \rom{H}(\cdot, \lambda \mathrm D^{[k]}u)$$ and aim at proving $\overline M<M$. Without loss of generality, we can assume $\overline M>0$. So, by definition of essential supremum, for every $\delta\in (\overline M/2,\overline M)$, there exists $x_\delta\in U$ such that
...
\begin{align*}
\esssup_{U\setminus U^\eps}\rom{H}(\cdot, \mathrm D^{[k]}u_\lambda)&=\esssup_{U\setminus U^\eps}\rom{H}(\cdot, \lambda\mathrm D^{[k]}u)\\
...
$$
\esssup_{U}\rom{H}(\cdot, \mathrm D^{[k]}u_\lambda)\leq \max\left\{\esssup_{U^\eps\cap U}\rom{H}(\cdot, \mathrm D^{[k]}u_\lambda), \esssup_{U\setminus U^\eps}\rom{H}(\cdot, \mathrm D^{[k]}u_\lambda)\right\}<M.
$$
This inequality and \eqref{u lambda} contradict the absolute minimality of $u$.\\

\noindent
\textit{Step 2:} General case.\\
Assume that $\rom{H}(x,\cdot)$ is radially non-decreasing for almost every $x\in\Om$. For $\eps>0$, consider the function $\rom{H}_\eps:\Om\times\R^\alpha\to\R^+$ defined as
$$
\rom{H}_\eps(x,X)=\rom{H}(x,X)+\eps|X|^2 \qquad\forall X\in\R^\alpha\quad\mbox{a.e. }x\in\Om.
$$
Then, $\rom{H}_\eps$ is strongly radially increasing in $X$, uniformly w.r.t. $x$ (see also Example \ref{Ex}): indeed, it is enough to take $c(t,\rho):=\eps(1-t^2)\rho^2$ as a modulus of monotonicity. hence, by \eqref{one ineq} and \textit{Step 1}, for every open subset $U\subseteq\Om$, we have
\begin{align}\label{eq h eps}
\esssup_U \rom{H}_\eps(\cdot, \mathrm D ^{[k]}u)=\sup_{\partial U}\rom{H}_\eps(\cdot,\mathrm D ^{[k]}u)^*.
\end{align}
Moreover, for every compact subset $K\subset\R^\alpha$, $\rom{H}_\eps\to \rom{H}$ uniformly in $\Om\times K$ as $\eps\to 0$. Therefore, recalling Lemma \eqref{lemma esssup}, we have
\begin{align*}
\sup_{\partial U}\rom{H}_\eps(\cdot,\mathrm D ^{[k]}u)^*&=\lim_{\rho\to0}\esssup_{(\partial U)^\rho\cap U}\rom{H}_\eps(\cdot, \mathrm D ^{[k]}u)\\
&\leq \inf_{\rho>0}\esssup_{(\partial U)^\rho\cap U}\rom{H}_\eps(\cdot, \mathrm D ^{[k]}u).
\end{align*}
In particular, for every $\rho>0$,
$$
\sup_{\partial U}\rom{H}_\eps(\cdot,\mathrm D ^{[k]}u)^*\leq \esssup_{(\partial U)^\rho\cap U}\rom{H}_\eps(\cdot, \mathrm D ^{[k]}u).
$$
Passing to the limit in both sides as $\eps\to 0$, by uniform convergence of $\rom{H}_\eps$ to $\rom{H}$, we get
$$
\lim_{\eps\to0}\sup_{\partial U}\rom{H}_\eps(\cdot,\mathrm D ^{[k]}u)^*\leq\esssup_{(\partial U)^\rho\cap U}\rom{H}(\cdot, \mathrm D ^{[k]}u)\qquad\forall \rho>0.
$$
Sending $\rho\to0$ in the right hand side, using again Lemma \eqref{lemma esssup}, we obtain
\begin{align}\label{eq1}
\lim_{\eps\to0}\sup_{\partial U}\rom{H}_\eps(\cdot,\mathrm D ^{[k]}u)^*\leq\sup_{\partial U}\rom{H}(\cdot,\mathrm D ^{[k]}u)^*.
\end{align}
On the other hand, by recalling \eqref{one ineq} and the uniform convergence of $\rom{H}_\eps$ to $\rom{H}$, we can write
\begin{align}\label{eq2}
\sup_{\partial U}\rom{H}(\cdot,\mathrm D ^{[k]}u)^*\leq\esssup_U \rom{H}(\cdot, \mathrm D ^{[k]}u)=\lim_{\eps\to0}\esssup_U \rom{H}_\eps(\cdot, \mathrm D ^{[k]}u).
\end{align}
Putting together \eqref{eq1} and \eqref{eq2}, identity \eqref{eq h eps} allows to conclude
$$
\esssup_U \rom{H}(\cdot, \mathrm D ^{[k]}u)=\sup_{\partial U}\rom{H}(\cdot,\mathrm D ^{[k]}u)^*.
$$
...
In this section, we prove a gradient maximum principle for $p$-harmonic maps. To our best knowledge, a result of this kind is not present in the literature, at least in the vectorial case. For the scalar one, a gradient maximum principle for quasilinear elliptic equations can be found in \cite[Chapter 15]{GT}. We refer also to \cite[Appendix B]{PWZ} regarding gradient maximum and minimum principles for $p$-harmonic potentials on convex rings in the plane. In the case of general $p$-harmonic maps, however, one can only hope for a gradient maximum principle to hold, since a minimum principle does not hold in general, as existing examples show (see \cite{BBDS}).\\
...
$$
\max_{\overline U}|Du|=\max_{\partial U}|Du|.
$$
\end{Theorem}
...
Moreover, we underline that Theorem \ref{thm1}, in the particular case when $E_\infty(u,U)=\esssup_U|Du|$, cannot be deduced from Theorem \ref{thm p} by passing to the limit as $p\to \infty$. Indeed, it is not known if vector valued absolute minimisers of $E_\infty$ can be approximated by $p$-harmonic maps (and this is one of the reasons why existence of absolute minimisers is still open in this case). Adding to the challenge, estimates in the proof of Theorem \ref{thm p} blow up as $p\to\infty$.
\end{Remark}
...
Now, we aim at deriving an elliptic differential inequality for the function
$$
f:=\frac1p{|\rom Du|^p}.
$$
Recall that the regularity theory of $p$-harmonic maps (see \cite{U}) ensures $u\in C^{1,\alpha}_\rom{loc}(\Om,\R^N)$. Then, we have $|\rom Du|^\frac p2\in W^{1,2}_\mathrm{loc}(\Om)\cap L^\infty_\rom{loc}(\Om)$, which implies that $f\in W^{1,2}_\rom{loc}(\Om)$ as well.
